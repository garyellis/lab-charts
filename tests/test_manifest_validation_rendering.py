"""Manifest-validation wire and terminal rendering tests."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "chart_manager.services.manifest_validation.progress",
...
def test_service_module_does_not_import_rich(module: str, attrs: tuple[str, ...]) -> None:
    ...
    pipeline through `ManifestValidationService` — must not drag a TUI library into the
...
def _mixed_run_result() -> RunResult:
    ...
    return RunResult(
        rows=(
            ...
            RowResult(
                row=WorklistRow(
                    chart="future-app", env="dev", release="future-app", namespace="lab-dev"
                ),
                phases={
...
def test_wire_outcome_preserves_selection_diagnostics() -> None:
    ...
    assert (
        "nothing to validate: requested filters selected no affected validation cases" in markdown
    )
