...
Race conditions and footguns discovered while bringing up the LGTM stack in a kind cluster via `chart-manager sandbox test`. Each entry: what you'll see, why it happens, and how it's resolved.
...
`services/sandbox.py` calls `upgrade_install(..., wait=False)`. The same bootstrap pattern exists in `mimir-distributed` and any other chart that ships a bundled minio + bucket-creation hook, so this is the right default for kind-based smoke tests.
...
**Fix in chart-manager**
Between install and test, `services/sandbox.py` calls `Kubectl.wait_workloads_ready(namespace, timeout)` (`integrations/kubectl.py`), which runs `kubectl rollout status` for every `deployment`, `statefulset`, and `daemonset` in the namespace.
...
**Fix in chart-manager**
`Helm.test` in `integrations/helm.py` no longer passes `--logs`. On test failure, `services/sandbox.py` already calls `Kubectl.diagnostics(namespace)` which dumps pods + events — enough to start debugging.
