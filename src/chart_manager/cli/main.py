...
# `-o table` is bound, not merely defaulted. `plan`'s output default became
# `auto` in P1.4, and `auto` resolves to json off a terminal -- which is
# exactly where this command runs (`.github/workflows/ci.yaml:213` captures it
# in `$( )` and reads it with `while IFS= read -r chart`). Binding the
# projection keeps the deprecated spelling byte-identical to what it always
# emitted, and puts the flag the replacement now needs into the deprecation
# line itself, so the caller migrating this in P1.7 is told what to type.
ALIASES.command(
    ci_app,
    plan,
    old="ci publish-charts",
    new="plan --for publish -o table",
    bind={"for_": "publish", "output": "table"},
)
