...
def list_charts(root: RootOption = Path(".")) -> None:
    ...
    console.print(table)
    if invalid:
        # The table is still printed: `chart list` answered the question,
        # and the rows that parsed are the useful part. What failed is the
        # *authoring* of a `Chart.yaml` or `chart-lifecycle.yaml`, which is
        # design §6.1's spec error -- exit 3, not the generic 1 this used to
        # return. `ChartCatalogService` is the only thing that knows which
        # charts are malformed, and it reports them by filling in `error`.
...
def grafana_export_dashboard(
    ...
    try:
        ...
        # Same shape as `_changed_paths`: an OSError from a path the caller
        # typed into a flag is a usage error naming that flag, not a
        # traceback. `-o some/existing/dir` was the traceback case -- the
        # write-side twin of the `lint-dashboards --path DIR` defect.
        raise typer.BadParameter(
            f"cannot write dashboard to {output}: {exc}",
            param_hint="--output",
        ) from exc
