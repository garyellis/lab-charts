...
from chart_manager.services.lab.models import AccessHints, _RunSummary
...
def lab_ca_present(summary: _RunSummary) -> bool:
    ...
    return any(
        entry.chart == LAB_CA_OWNER_CHART
...
def wait_apps_wildcard_ready(
    ...
    *,
    ...
    progress(
        step(
            "Waiting for",
            f"Certificate/{APPS_WILDCARD_CERT_NAME} "
...
def urls_and_grafana_host(hosts: Sequence[str]) -> tuple[tuple[str, ...], str | None]:
    ...
    grafana_host = next(
        (h for h in ordered if h.startswith(f"{GRAFANA_RELEASE}.")), None
    )
...
def access_hints(
    ...
    *,
    ...
    Accumulates into locals and constructs `AccessHints` at exactly one
    exit. There used to be five `return AccessHints(...)` sites, each
    ...
    if hosts:
        ...
        if grafana_host is not None:
            ...
            # and the two coincide today only because the grafana test-spec
