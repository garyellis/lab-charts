...
share three hand-threaded mutable accumulators (`_RunSummary`,
...
from chart_manager.services.lab.drift import (
...
class LabService:
    ...
    def up(self, options: LabUpOptions) -> LabResult:
        ...
        self.kind.ensure_cluster(
            options.cluster_name, config=kind_config_path(self.root)
        )
        ...
        return summary.freeze(
            self._access_hints(summary, namespace=options.namespace)
        )

    def sync(self, options: LabSyncOptions) -> LabResult:
        ...
        *,
        ...
        if cilium_installed and options.skip_installed and not force:
            ...
            summary.no_change.append(
                EntryOutcome(CILIUM_BOOTSTRAP_CHART, "minimal", CILIUM_BOOTSTRAP_NAMESPACE)
            )
    ...
    def down(self, cluster_name: str = DEFAULT_CLUSTER_NAME) -> ClusterActionResult:
    ...
    def delete(self, cluster_name: str = DEFAULT_CLUSTER_NAME) -> ClusterActionResult:
    ...
    def _existing_release_keys(self) -> set[tuple[str, str]]:
        ...
        try:
            ...
            self._progress(
                warn(
                    f"could not list helm releases ({exc}); "
        ...
        *,
        ...
        for entry in plan:
            ...
            try:
                ...
                self._progress(
                    failure("profile resolution failed:", f"{entry.chart}: {exc}")
                )
                summary.failed.append(
                    EntryFailure(
                        chart=entry.chart,
            ...
            if skip_installed and key in installed_keys:
                ...
                summary.no_change.append(
                    EntryOutcome(entry.chart, entry.profile, namespace)
                )
            ...
            try:
                ...
                self._progress(
                    step("Applying", f"{entry.chart}:{entry.profile} -> {namespace}")
                )
                ...
                if applied:
                    ...
                    self._progress(
                        detail("no change", f"{entry.chart} (rollout wait skipped)")
                    )
                bucket = summary.applied if applied else summary.no_change
                bucket.append(EntryOutcome(entry.chart, entry.profile, namespace))
                installed_keys.add(key)
            except (ExternalCommandError, ChartManagerError) as exc:
                self._progress(
                    failure(
                        "apply failed:", f"{entry.chart}:{entry.profile} -> {exc}"
                    )
                )
                summary.failed.append(
                    EntryFailure(
    ...
    # is what lets those modules stay free of `LabService` while the converge
    ...
    def _wait_apps_wildcard_ready(self, summary: _RunSummary) -> None:
        ...
        wait_apps_wildcard_ready(
            summary, kubectl=self.kubectl, progress=self._progress
        )

    def _access_hints(self, summary: _RunSummary, *, namespace: str) -> AccessHints:
