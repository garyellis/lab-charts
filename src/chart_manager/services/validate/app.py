"""Transport-neutral entry point for the validate pipeline.

`ValidateApp` is the capability behind `chart-manager validate
...
It does NOT own what a row *is*. Which (chart, env) pairs exist, which
survive the chart/env filters, and what each one runs with (chart path,
values, policy dirs) all belong to `worklist.py` — one module, one answer
to "why did this row get these values files?".
...
Deliberately Rich-free, like `services/validate/wire.py`: a REST worker or
...
from chart_manager.services.validate.runner import (
    EventCallback,
    RowConfig,
    ValidateRunner,
)
...
class RunnerSpec:
    """Everything needed to build one `ValidateRunner`.
...
RunnerFactory = Callable[[RunnerSpec], ValidateRunner]
...
class ValidateApp:
    ...
    def _build_runner(self, spec: RunnerSpec) -> ValidateRunner:
