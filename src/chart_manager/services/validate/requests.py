"""The caller-facing vocabulary of the validate capability.
...
`__post_init__`, so an ill-formed request cannot reach `ValidateApp` — the
...
class SingleRequest:
    ...
    Single-row requests do NOT consult `validate-spec.yaml`; callers pass
...
class RunOutcome:
    ...
    `result` is the wire-projectable payload (`services/validate/wire.py`);
