...
def check_cilium_service_host_drift(
    ...
    *,
    ...
    try:
        ...
        progress(warn(f"could not read control-plane IP for drift check ({exc}); skipping"))
        return

    try:
        values = helm.get_values(CILIUM_BOOTSTRAP_CHART, namespace=CILIUM_BOOTSTRAP_NAMESPACE)
    except ExternalCommandError as exc:
        progress(warn(f"could not read cilium release values for drift check ({exc}); skipping"))
        return

    installed_ip = _walk(values, CILIUM_SERVICE_HOST_PATH)

    if installed_ip is None:
        progress(warn("cilium release has no pinned k8sServiceHost; skipping drift check"))
        return
