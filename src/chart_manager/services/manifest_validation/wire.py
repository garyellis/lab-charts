...
def _markdown_diagnostics(diagnostics: dict[str, object]) -> list[str]:
    ...
    if requested["environments"]:
        lines.append(f"- Requested environments: {', '.join(requested['environments'])}")
    if unmatched["charts"]:
        lines.append(f"- Unmatched charts: {', '.join(unmatched['charts'])}")
    if unmatched["environments"]:
        lines.append(f"- Unmatched environments: {', '.join(unmatched['environments'])}")
    ...
    if selection["charts_unvalidated"]:
        lines.append(
            f"- Charts without manifest-validation configuration: {selection['charts_unvalidated']}"
        )
    return lines
    ...
    *,
    ...
    if not result.rows:
        if source.unmatched_charts or source.unmatched_environments:
            ...
            no_work_reason = "no chart with manifest-validation configuration was selected"
        else:
